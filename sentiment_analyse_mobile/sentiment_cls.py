        ...
        for i in range(len(sents_list)):
            ...
            if set_type == "test":
                ...
                for ii in range(len(labels_list[i])):
                    labels.append(tokenization.convert_to_unicode(labels_list[i][ii]))
                label = labels
            examples.append(
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  label_id = list(map(lambda x:label_map[x], example.label))
  if ex_index < 5:
    ...
    tf.logging.info("label: {} (id = {})".format(example.label, str(label_id)))
...
def file_based_input_fn_builder(input_file, seq_length, cls_num, is_training,
                                drop_remainder):
  ...
  name_to_features = {
      ...
      "label_ids": tf.FixedLenFeature([cls_num], tf.int64),
  }
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    for i in range(label_num):
        with tf.variable_scope('cls_{}'.format(i)):
    ...
    loss = tf.reduce_mean(tf.convert_to_tensor(loss_list))
    return (loss, per_example_loss_list, logits_list, probabilities_list)
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits):
        ...
        predictions = tf.argmax(tf.reshape(tf.transpose(tf.convert_to_tensor(logits), (1, 0, 2)), shape=[-1, 4]), axis=-1,
                  output_type=tf.int32)
        accuracy = tf.metrics.accuracy(tf.reshape(label_ids, [-1]), predictions)
